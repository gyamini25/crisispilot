...
def build_deployments_dql(service: str | None) -> str:
    """DQL for recent deployment events in Grail (last 24h).

    On a tenant with no ingested deployment events this returns zero rows — the
    DeploymentAgent then falls back to its scripted commit narrative.
    """
    return (
...
class DynatraceMCP:
    ...
    async def query_deployments(self, service: str | None) -> dict[str, Any] | None:
        """High-level helper: fetch recent deployment events from Grail."""
        dql = build_deployments_dql(service)
        records = await self.execute_dql(dql)
        if records is None:
            return None
        return {"rows": len(records), "dql": dql, "records": records[:5]}
